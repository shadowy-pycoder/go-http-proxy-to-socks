...
- **TCP and UDP Transparent proxy**\
  `tproxy` and `tproxylocal` (IP_TRANSPARENT) handle TCP and UDP traffic
...
OPTIONS:
  ...
  -M         Transparent proxy mode: (redirect, tproxy, tproxylocal)
  ...
  -mark      Set mark for each packet sent through transparent proxy (Default: redirect 0, tproxy 100, tproxylocal 100)
...
There are three modes `redirect`, `tproxy` and `tproxylocal` (same as `tproxy` but also intercepts local traffic) that can be specified with `-M` flag
...
transparent_proxy:
  ...
  udp:
    ...
    workers: 1
  mode: "tproxy" # available modes are "redirect", "tproxy" and "tproxylocal" (udp requires tproxy or tproxylocal mode)
  ...
  ignored_ports: []
  # set mark for each packet sent through transparent proxy (Default: redirect 0, tproxy 100, tproxylocal 100)
  mark: 100
```
