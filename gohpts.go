...
type Proxy struct {
	...
	allowedIPv4 string
	// list of allowed IPv6 addresses
	allowedIPv6  string
	ignoredPorts string
...
func New(conf *Config) (*Proxy, error) {
	...
	if conf.TProxyMode != "" && (conf.TProxy != "" || conf.TProxyUDP != "") {
		...
		// parsing whitelisted ip addresses
		if conf.AllowedIPs != "" {
			if !p.auto {
				return nil, fmt.Errorf("allowing ip addresses is only possible in auto configuration")
			}
			var allowedIPv4, allowedIPv6 []string
			for a := range strings.SplitSeq(conf.AllowedIPs, ",") {
				addr, err := netip.ParseAddr(a)
				if err != nil {
					prefix, err := netip.ParsePrefix(a)
					if err != nil {
						return nil, fmt.Errorf("allowed ip addresses list is malformed: %v", err)
					}
					if prefix.Addr().Is4() {
						allowedIPv4 = append(allowedIPv4, prefix.String())
					} else {
						allowedIPv6 = append(allowedIPv6, prefix.String())
					}
					continue
				}
				if addr.Is4() {
					allowedIPv4 = append(allowedIPv4, addr.String())
				} else {
					allowedIPv6 = append(allowedIPv6, addr.String())
				}
			}
			if p.ipv4enabled {
				p.allowedIPv4 = strings.Join(allowedIPv4, ",")
			}
			if p.ipv6enabled {
				p.allowedIPv6 = strings.Join(allowedIPv6, ",")
			}
			if p.allowedIPv4 != "" && p.allowedIPv6 == "" {
				p.ip = "ip4"
				p.tcp = "tcp4"
				p.udp = "udp4"
				p.ipv6enabled = false
				p.ipv4only = true
				p.ipv6only = false
			} else if p.allowedIPv6 != "" && p.allowedIPv4 == "" {
				p.ip = "ip6"
				p.tcp = "tcp6"
				p.udp = "udp6"
				p.ipv4enabled = false
...
func (p *Proxy) Run() error {
	...
	if p.arpSpoofConf != "" && !p.ipv4enabled {
		...
		if gwIP := net.ParseIP(gw.String()); gwIP != nil {
			p.gwDNS = &net.UDPAddr{IP: gwIP, Port: 53}
		}
		if p.dns != nil {
			p.afDNS = p.dns
		} else if p.nsEnabled {
	...
	if p.dns != nil {
		p.logger.Info().Msgf("DNS: %s", p.dns)
	}
	if p.outDNS != nil {
